...
class Trainer:
    ...
    def train_eval_all_epoches(self,epoch):
        ...
        self.model.eval()
        self.best_results = self.inferece_per_epoch(self.model,self.best_results,suffix='n',epoch=epoch)
        self.best_results_ema = self.inferece_per_epoch(self.model_ema,self.best_results_ema,suffix='s',epoch=epoch)
    ...
    def inferece_per_epoch(self,model,best,suffix,epoch):
        ...
        # 构造文件名，包含epoch编号
        save_name = f"{self.config['name']}_epoch_{epoch}_head_{self.args.test_set}"
        # 保存模型状态字典和验证结果
        torch.save(
            {"state_dict": state_dict, "validation_results": best, },
            f"{self.args.resume}/{save_name}_{suffix}_finetuned.pth"
